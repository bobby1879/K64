...
/*
 * Task can be created in blocked state.
 * The _task_ready() will move it into ready queue.
 */
#define MQX_TASK_CREATE_BLOCKED        (0x20)
 ...
 *                         MQX MEMORY TYPES
 */
#define MEM_TYPE_SYSTEM_STACK             1       // Obsolete, not used anymore.
...
/*! \cond DOXYGEN_IGNORE */
typedef void (_CODE_PTR_ INT_EXCEPTION_FPTR)(_mqx_int, _mqx_uint, void *, void *);
/*! \endcond */
...
typedef struct   td_struct
{
   ...
   void* FIRST_OWNED_BLOCK;
   #endif  /* MQX_ALLOCATOR_GARBAGE_COLLECTING */
#endif /* MQXCFG_ALLOCATOR == MQX_ALLOCATOR_TLSF */
...
extern INT_ISR_FPTR     _int_get_isr(_mqx_int);
extern void            *_int_get_isr_data(_mqx_int);
...
extern void            *_int_set_isr_data(_mqx_int, void *);
...
extern INT_EXCEPTION_FPTR _int_get_exception_handler(_mqx_int);
extern INT_EXCEPTION_FPTR _int_set_exception_handler(_mqx_int,INT_EXCEPTION_FPTR);
#endif /* MQXCFG_EXCEPTION_HANDLING */

#if (MQXCFG_ALLOCATOR == MQX_ALLOCATOR_MEM) || (MQXCFG_ALLOCATOR == MQX_ALLOCATOR_LWMEM)
...
#endif /* (MQXCFG_ALLOCATOR == MQX_ALLOCATOR_MEM) || (MQXCFG_ALLOCATOR == MQX_ALLOCATOR_LWMEM) */
...
#else /* MQX_USE_MEM_ZERO */
# include <string.h>
#define _mem_zero(p,n)  memset(p,0,n)
#endif /* MQX_USE_MEM_ZERO */
...
#if MQX_CUSTOM_MAIN
extern _mqx_uint        mqx_init(register const MQX_INITIALIZATION_STRUCT *mqx_init);
extern _mqx_uint        mqx_start(void);
#else
...
/*--------------------------------------------------------------------------*/
/*
 *                  STATIC STACK SIZE MACRO
 */

#define MQX_REQUIRED_STACK_SIZE(size) (sizeof(TD_STRUCT) + (size) + (PSP_STACK_ALIGNMENT) + 1)
